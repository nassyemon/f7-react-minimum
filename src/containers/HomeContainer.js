...
import { addPicture, clearPicture, openWebApiCamera } from '../actions/CameraActions';
...
const mapDispatchToProps = (dispatch) => {
  return {
    ...
    onClickCameraButton: () => getPicture(Camera.PictureSourceType.CAMERA, Camera.DestinationType.FILE_URI, dispatch),
    onClickAlbumButton: () => getPicture(Camera.PictureSourceType.PHOTOLIBRARY, Camera.DestinationType.FILE_URI, dispatch),
...
function getPicture(sourceType, destinationType, dispatch) {
...
function onSuccess(dispatch) {
  return (fileUri) => {
    console.log(fileUri);
    dispatch(addPicture(fileUri));
  }
}

function onFail(dispatch) {
  return (message) => {
    alert(message);
  }
}
